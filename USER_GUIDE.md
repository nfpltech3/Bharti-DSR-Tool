...
1. **Checklist Parser (`Checklist_Parser_App.exe`):** A dual-purpose application that extracts job data from PDFs to push into Shakti Pre-Alert, and formats raw DSR reports into clean, client-ready Excel sheets.
2. **Excel Importer (`standalone_excel_import.exe`):** A tool for merging Excel data for manual DSR reports and pushing subform items into Shakti.
...
### Tab 1: Checklist Parser (PDF to Shakti)
This tab allows you to parse customs checklist PDFs and push the extracted information to Shakti Pre-Alert.
...
4. **Push to Shakti:** Click **PUSH TO SHAKTI PRE-ALERT**. The app will create a new record or update an existing one if a matching Job No / HAWB is found. Successful submissions are logged locally in `prealert_submission_log.csv`.
5. **Next File:** The app will automatically queue the next PDF for review. You can also use **SKIP FILE** to bypass a PDF.

### Tab 2: Format DSR (Shakti Excel to Client DSR)
This tab transforms raw Shakti exported tabular reports into neatly formatted, multi-sheet Excel reports ready for the client (Bharti).

1. **Format:** Click **SELECT & FORMAT DSR EXCEL** and choose your raw Shakti exported Excel file.
2. **Automated Processing:** The tool will automatically:
   ...
   * Rename internal Shakti headers to standard client-facing headers based on the job's **FTWZ STATUS**.
...
2. **Merge DSRs (Tab 1):** Click **SELECT FILES & MERGE**. Upload the exported Shakti DSR report first, followed by your Manual DSR Excel file. The tool will automatically match the columns.
3. **Import Subforms (Tab 2):** Click **SELECT SUBFORM EXCEL & PUSH** to upload an Excel file and push subform items directly into Shakti.
...
| :--- | :--- | :--- |
| `HTTP 401 / 403` | Shakti token expired or missing. | Ensure your `.env` is correct. The app will auto-refresh the token if configured properly. |
...
| `Could not find 'FTWZ Status' column` | DSR formatting failed. | Ensure the Shakti export includes the "FTWZ Status" column. |
